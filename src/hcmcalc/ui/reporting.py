...
SUPPORTED_CALCULATION_TYPES = {"manual_single_segment", "manual_facility_v0"}
...
def report_filename(report: dict[str, Any], extension: str) -> str:
    ...
    workflow = "single_segment" if calculation_type == "manual_single_segment" else "facility"
...
def report_to_csv(report: dict[str, Any]) -> str:
    ...
    writer.writerow(["HCM7 Chapter 15 Report"])
...
def _audit_summary(result: dict[str, Any], audit_record: dict[str, Any] | None) -> list[dict[str, Any]]:
    ...
    for path, value in _flatten(audit_record or {}):
        lowered = path.lower()
        if any(term in lowered for term in ("source", "reference", "validation_basis", "scope_status", "template_id")):
...
def _validate_result(calculation_type: str, result: dict[str, Any] | None) -> None:
    ...
    required = (
        ...
        if calculation_type == "manual_single_segment"
        else {"facility_level_of_service", "facility_follower_density_followers_mi_ln", "facility_average_speed_mph", "facility_length_mi", "segments"}
    )
