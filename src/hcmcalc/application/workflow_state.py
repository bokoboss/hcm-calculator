"""Framework-independent calculation freshness and presentation state.

This module is the canonical application copy of the existing fingerprint
contract.  The legacy Streamlit module re-exports these symbols so existing
callers keep the same behavior while the rebuilt application has a stable
framework-neutral import boundary.
"""
...
class ResultPresentationState(StrEnum):
    """Canonical states used to present an application calculation outcome."""
...
def resolve_result_presentation_state(
    *,
    ...
    """Classify presentation state without altering an engine result."""
...
def normalized_input_fingerprint(inputs: Mapping[str, Any]) -> str:
    """Return a stable fingerprint without mutating supplied inputs."""
...
def calculation_input_fingerprint(
    ...
    """Fingerprint method identity, input contract, and normalized inputs."""
...
def mark_calculated(
    ...
    """Capture a successful calculation fingerprint in session state only."""
...
def is_current_calculation(
    ...
    *,
    ...
    """Whether a stored result may be represented as current."""
