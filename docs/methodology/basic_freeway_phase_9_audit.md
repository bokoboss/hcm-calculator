...
The checked-in `BF-CH26-001` reference case uses SAF = CAF = 1.0, so its expected output is unchanged. Non-unity regression coverage verifies the four SAF/CAF combinations, the exact measured-FFS case (65 mi/h, SAF 0.950, CAF 0.939: base capacity 2,350 and adjusted capacity 2,206.65 pc/h/ln), Metric UI normalization, and the near-capacity status transition. For the transition case, 5,945 veh/h independently computes to 2,190.2631579 pc/h/ln, between the old defective 2,176.1325 and corrected 2,206.65 thresholds; the old path would be LOS F with null speed/density, while the corrected path predicts speed and density. No checked-in Basic Freeway preset or Chapter 26 fixture uses a non-unity pair.
