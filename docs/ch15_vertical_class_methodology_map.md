...
| `grade_length_mi` | Legacy saved-project/import compatibility field. When present it is retained for audit normalization, but it does not drive classification: Exhibit 15-11 uses `segment_length_mi`. A distinct grade-transition input is not implemented. |
...
| `segment_length_mi` | The Exhibit 15-11 analysis-segment-length input and the Step 1 Exhibit 15-10 applicability input. It is also used directly by class-specific speed and percent-followers equations in separately guarded downstream calculations. |
...
| Level | Passing Lane | `0%` | `0.25-3.0 mi` | 1 | Exactly `8%` manually | Chapter 26 Example 3; level reliability matrix | Implemented, narrowly validated | Step 7 Eq. 15-24 through Eq. 15-33 are centralized and auditable; manual result excludes downstream/facility effects. |
