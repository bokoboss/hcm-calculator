...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Step 2 | Demand flow, capacity, and demand-to-capacity context | Eq. 15-1; Chapter 15 capacity text | Passing Constrained and Passing Zone use audited 1,700 veh/h/ln base capacity. Demand is never capped; over-capacity is flagged and LOS is F. | Supported in Manual Single Segment. | Phase 2 capacity-boundary, unit, and published-example regression coverage. | Passing Lane capacity remains Phase 3 scoped. | Facility application remains Phase 3. |
...
| Step 7 | Passing Lane measures | Eq. 15-24 through Eq. 15-33 | Centralized and auditable across applicable Classes 1--5, including lane flow/HV conservation. | Standalone midpoint result; no facility aggregate. | Focused equation, context, and Examples 3/4 regression tests. | UI remains a single-segment worksheet. | Phase 4 facility UI. |
...
| Step 9 | Downstream Passing Lane adjustment | Eq. 15-36 through Eq. 15-38 | Explicit ordered engine contexts identify the closest upstream Passing Lane and effective-length boundary. | Not exposed in Manual Single Segment. | Focused sequence/boundary tests and Examples 3/4 regression tests. | Step 11 aggregation and arbitrary facility UI remain out of scope. | Phase 4 facility UI. |
