...
def _displayed_inputs(method_id: str, template_id: str, unit_system: str) -> dict[str, Any]:
    starting = workflow_for_method(method_id).starting_values(template_id, unit_system)
...
def _assert_pair_semantics(metric: dict[str, Any], imperial: dict[str, Any]) -> None:
    _assert_equivalent(metric["normalized_inputs"], imperial["normalized_inputs"], "normalized")
    _assert_equivalent(metric["result"], imperial["result"], "result")
    assert metric["calculation_state"]["presentation_state"] == imperial["calculation_state"]["presentation_state"]
    assert metric["presentation"]["answer"]["available"] is imperial["presentation"]["answer"]["available"]
    assert metric["presentation"]["capacity"]["failure"] is imperial["presentation"]["capacity"]["failure"]
...
def test_metric_imperial_equivalence_through_application_workflows(
    ...
    metric = workflow.calculate(
        ...
        unit_system="metric",
        displayed_inputs=_displayed_inputs(method_id, template_id, "metric"),
    )
    imperial = workflow.calculate(
        template_id=template_id,
        unit_system="imperial",
        displayed_inputs=_displayed_inputs(method_id, template_id, "imperial"),
    )
...
def test_metric_normalization_does_not_round_before_engine_decisions() -> None:
    multilane = _displayed_inputs("multilane_segment", "MLH-CH26-004-EB", "metric")
    multilane.update(
        {
    ...
    for key, expected in {
        "segment_length_ft": multilane["segment_length"] * 1000.0 / FEET_TO_METERS,
        "posted_speed_limit_mph": multilane["posted_speed_limit"] * (1.0 / MILES_TO_KILOMETERS),
        "lane_width_ft": multilane["lane_width"] / FEET_TO_METERS,
        "roadside_lateral_clearance_ft": multilane["roadside_lateral_clearance"] / FEET_TO_METERS,
        "access_point_density_per_mi": multilane["access_point_density"] * MILES_TO_KILOMETERS,
        "left_side_lateral_clearance_ft": multilane["left_side_lateral_clearance"] / FEET_TO_METERS,
    }.items():
        assert multilane_normalized[key] == expected

    freeway = _displayed_inputs("basic_freeway_segment", "BF-CH26-001", "metric")
    freeway.update(
        {
            "segment_length": 1.23456789,
    ...
    for key, expected in {
        "segment_length_mi": freeway["segment_length"] * (1.0 / MILES_TO_KILOMETERS),
        "base_free_flow_speed_mph": freeway["base_free_flow_speed"] * (1.0 / MILES_TO_KILOMETERS),
        "lane_width_ft": freeway["lane_width"] / FEET_TO_METERS,
        "right_side_lateral_clearance_ft": freeway["right_side_lateral_clearance"] / FEET_TO_METERS,
        "total_ramp_density_per_mi": freeway["total_ramp_density"] * MILES_TO_KILOMETERS,
    }.items():
        assert freeway_normalized[key] == expected
...
def test_two_lane_metric_imperial_pair_preserves_exhibit_length_boundary() -> None:
    ...
    imperial_inputs = _displayed_inputs(method_id, template_id, "imperial")
    ...
    _assert_pair_semantics(metric, imperial)
    assert metric["result"]["outputs"]["segment_length_applicability_status"] == "within_exhibit_15_10"
    assert imperial["result"]["outputs"]["segment_length_applicability_status"] == "within_exhibit_15_10"
...
def _near_capacity_pair(
    ...
    capacity = outputs.get("adjusted_capacity_pc_h_ln", outputs.get("capacity_pc_h_ln"))
    flow_per_demand = outputs["demand_flow_rate_pc_h_ln"] / imperial_inputs["demand_volume_veh_h"]
...
@pytest.mark.parametrize("ratio, capacity_failure", [(0.999, False), (1.001, True)])
def test_multilane_near_capacity_pair_preserves_left_side_hcm_source(
    ...
    metric, imperial = _near_capacity_pair(
        "multilane_segment",
        "MLH-CH26-004-EB",
        {"median_type": "divided", "left_side_lateral_clearance": 6.0 * FEET_TO_METERS},
        {"median_type": "divided", "left_side_lateral_clearance": 6.0},
        ratio,
    )
    _assert_pair_semantics(metric, imperial)
    assert metric["normalized_inputs"]["left_side_lateral_clearance_ft"] == pytest.approx(6.0)
    assert imperial["normalized_inputs"]["left_side_lateral_clearance_ft"] == pytest.approx(6.0)
    for response in (metric, imperial):
        outputs = response["result"]["outputs"]
        assert outputs["capacity_status"] == (
            "demand_exceeds_capacity" if capacity_failure else "within_capacity"
        )
...
def test_basic_freeway_saf_caf_and_near_capacity_pair(
    ...
    metric, imperial = _near_capacity_pair(
        "basic_freeway_segment",
        "BF-CH26-001",
        {"ffs_source": "measured", "free_flow_speed": 65.0 * MILES_TO_KILOMETERS, "driver_population_category": "mostly_unfamiliar"},
        {"ffs_source": "measured", "free_flow_speed": 65.0, "driver_population_category": "mostly_unfamiliar"},
        ratio,
    )
    _assert_pair_semantics(metric, imperial)
    for response in (metric, imperial):
        ...
        assert outputs["adjusted_capacity_pc_h_ln"] == pytest.approx(2350.0 * 0.898)
...
def test_weaving_lmax_handoff_pair_preserves_null_and_side_semantics() -> None:
    ...
    handoff_length = baseline["result"]["outputs"]["maximum_weaving_length_ft"] + 1.0
    metric_inputs = _displayed_inputs(method_id, template_id, "metric")
    ...
    _assert_pair_semantics(metric, imperial)
    for response in (metric, imperial):
        assert response["result"]["outputs"]["support_status"] == "hcm_handoff_required"
        assert response["result"]["outputs"]["capacity_status"] == "not_evaluated_after_handoff"
        assert response["result"]["outputs"]["level_of_service"] is None
        assert response["calculation_state"]["presentation_state"] == "hcm_stopping_or_handoff"
