...
test.describe('Phase 3 full migration', () => {
  test('all seven registered methods are actionable in the rebuilt launcher', async ({ page }) => {
    await page.goto('/');
    await page.getByRole('button', { name: 'New Analysis' }).first().click();
    await expect(page.locator('.delivery-count')).toHaveText('7');
    for (const methodId of [
      ...
      await expect(page.getByTestId(`method-card-${methodId}`).getByRole('button', { name: 'Select method' })).toBeEnabled();
    }
  });

  test('each Phase 3 workflow calculates and exposes release evidence', async ({ page }) => {
    for (const methodId of phase3Methods) {
      await page.goto('/');
      await page.getByRole('button', { name: 'New Analysis' }).first().click();
      await page.getByTestId(`method-card-${methodId}`).getByRole('button', { name: 'Select method' }).click();
  ...
  test('weaving length handoff keeps unavailable metrics distinct from capacity failure', async ({ page }) => {
    await page.goto('/');
    await page.getByRole('button', { name: 'New Analysis' }).first().click();
    await page.getByTestId('method-card-weaving_segment').getByRole('button', { name: 'Select method' }).click();
